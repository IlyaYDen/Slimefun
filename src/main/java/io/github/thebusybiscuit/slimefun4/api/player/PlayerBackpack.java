...
public class PlayerBackpack {
    ...
    public PlayerBackpack(@Nonnull PlayerProfile profile, int id, int size) {
        if (size < 9 || size > 54 || size % 9 != 0) {
            throw new IllegalArgumentException("Недопустимый размер! Размер должен быть одним из: [9, 18, 27, 36, 45, 54]");
        ...
        inventory = Bukkit.createInventory(null, size, "Backpack [" + size + " слотов]");
    }
    ...
    public void setSize(int size) {
        ...
        Inventory inv = Bukkit.createInventory(null, size, "Рюкзак [" + size + " слотов]");
