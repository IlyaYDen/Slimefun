...
public class CoolerFeedPlayerEvent extends PlayerEvent implements Cancellable {
    ...
    public void setConsumedItem(@Nonnull ItemStack item) {
        Validate.notNull(item, "Товар должен существовать!");//The consumed Item cannot be null
        Validate.isTrue(item.getItemMeta() instanceof PotionMeta, "Предмет должен быть зельем!");//edited The item must be a potion
