...
public abstract class Network {
    ...
    protected Network(@Nonnull NetworkManager manager, @Nonnull Location regulator) {
        ...
        Validate.notNull(manager, "Должен быть сетевой менеджер");
        Validate.notNull(regulator, "Регулятор не был указан");//No regulator was specified
    ...
    protected void addLocationToNetwork(@Nonnull Location l) {
        Validate.notNull(l, "Вы не можете добавить местоположение в сеть, которое равно нулю!");
        //Validate.notNull(l, "You cannot add a Location to a Network which is null!");
        Validate.isTrue(l.getWorld().getUID().equals(worldId), "Сети не могут существовать в нескольких мирах!");
    ...
    public boolean connectsTo(@Nonnull Location l) {
        Validate.notNull(l, "Местоположение не может быть пустым.");
