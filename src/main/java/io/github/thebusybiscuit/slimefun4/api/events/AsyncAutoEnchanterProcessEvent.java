...
public class AsyncAutoEnchanterProcessEvent extends Event implements Cancellable {
    ...
    public AsyncAutoEnchanterProcessEvent(@Nonnull ItemStack item, @Nonnull ItemStack enchantedBook, @Nonnull BlockMenu menu) {
        super(true);
        //edited
        Validate.notNull(item, "Предмет, который нужно зачаровать, должен существовать!");
        Validate.notNull(enchantedBook, "Зачарованная книга должена существовать!");
        Validate.notNull(menu, "Меню Автоматического Стола Зачарования не может быть пустым!");
