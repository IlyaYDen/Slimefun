...
public class IntRangeSetting extends ItemSetting<Integer> {
    ...
    public IntRangeSetting(SlimefunItem item, String key, int min, int defaultValue, int max) {
        ...
        Validate.isTrue(defaultValue >= min && defaultValue <= max, "Значение по умолчанию не находится в диапазоне.");//The default value is not in range
    ...
    protected String getErrorMessage() {
        return "Только целые числа из " + min + '-' + max + "(включительно) допускаются!";
        //edited return "Only whole numbers from " + min + '-' + max + "(inclusive) are allowed!";
    }
