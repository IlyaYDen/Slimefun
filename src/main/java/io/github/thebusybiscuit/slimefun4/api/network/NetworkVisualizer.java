...
class NetworkVisualizer implements Runnable {
    ...
    NetworkVisualizer(@Nonnull Network network, @Nonnull Color color) {
        Validate.notNull(network, "Сеть не должна быть нулевой.");//The network should not be null
        Validate.notNull(color, "Цвет не может быть нулевым.");//The color cannot be null
