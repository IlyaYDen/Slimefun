...
public final class Script {
    ...
    ItemStack getAsItemStack(@Nonnull ProgrammableAndroid android, @Nonnull Player p) {
        ...
        lore.add("");
        lore.add("&7Загрузок: &f" + getDownloads());
        lore.add("&7Рейтинг: " + getScriptRatingPercentage());
        lore.add("&a" + getUpvotes() + " \u263A &7| &4\u2639 " + getDownvotes());
        lore.add("");
        lore.add("&eЛевый Клик &fчтобы скачать скрипт");
        lore.add("&4(это удалит нынешний скрипт)");

        if (canRate(p)) {
            lore.add("");
            lore.add("&eShift + Левый Клик, &fчтобы поставить положительную оценку");
            lore.add("&eShift + Правый Клик, &fчтобы поставить отрицателную оценку");
        }
