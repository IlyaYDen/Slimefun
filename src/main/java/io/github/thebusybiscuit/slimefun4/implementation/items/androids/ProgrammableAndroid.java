...
public class ProgrammableAndroid extends SlimefunItem implements InventoryBlock, RecipeDisplayItem {
    ...
    public ProgrammableAndroid(Category category, int tier, SlimefunItemStack item, RecipeType recipeType, ItemStack[] recipe) {
        ...
        new BlockMenuPreset(getId(), "Программируемый Андроид") {
            ...
            public void newInstance(BlockMenu menu, Block b) {
                menu.replaceExistingItem(15, new CustomItem(HeadTexture.SCRIPT_START.getAsItemStack(), "&aНачать/Продолжить"));
                ...
                menu.replaceExistingItem(17, new CustomItem(HeadTexture.SCRIPT_PAUSE.getAsItemStack(), "&4Пауза"));
                ...
                menu.replaceExistingItem(16, new CustomItem(HeadTexture.ENERGY_REGULATOR.getAsItemStack(), "&bЯдро памяти", "", "&8\u21E8 &7Нажмите, чтобы открыть редактор скриптов"));
    ...
    public AndroidFuelSource getFuelSource() {
        switch (getTier()) {
            ...
            default:
                throw new IllegalStateException("Не удается преобразовать следующий уровень андроида в тип топлива: " + getTier());
    ...
    public void openScript(Player p, Block b, String sourceCode) {
        ...
        menu.addItem(0, new CustomItem(Instruction.START.getItem(), SlimefunPlugin.getLocalization().getMessage(p, "android.scripts.instructions.START"), "", "&7\u21E8 &eЛевый Клик &7чтобы вернуться к интерфейсу Андроид"));
        ...
        for (int i = 1; i < script.length; i++) {
            ...
            if (i == script.length - 1) {
                ...
                if (hasFreeSlot) {
                    menu.addItem(i, new CustomItem(HeadTexture.SCRIPT_NEW.getAsItemStack(), "&7> Добавить новую команду"));
                ...
                menu.addItem(slot, new CustomItem(Instruction.REPEAT.getItem(), SlimefunPlugin.getLocalization().getMessage(p, "android.scripts.instructions.REPEAT"), "", "&7\u21E8 &eЛевый Клик, &7чтобы вернуться к интерфейсу Андроида"));
                ...
                menu.addItem(i, new CustomItem(stack, SlimefunPlugin.getLocalization().getMessage(p, "android.scripts.instructions." + instruction.name()), "", "&7\u21E8 &eЛевый Клик &7для изменения", "&7\u21E8 &eПравый Клик &7чтобы удалить", "&7\u21E8 &eShift + Правый Клик &7чтобы дублировать"));
                menu.addMenuClickHandler(i, (pl, slot, item, action) -> {
    ...
    protected void openScriptDownloader(Player p, Block b, int page) {
        ChestMenu menu = new ChestMenu("Скрипты для андроида");
        ...
        for (int i = 45; i < 54; i++) {
            ...
            ItemStack it = new CustomItem(new ItemStack(Material.GRAY_STAINED_GLASS_PANE), " ");
        ...
        ItemStack it = new CustomItem(HeadTexture.SCRIPT_UP.getAsItemStack(), "&eЗагрузить скрипт", "", "&6Клик &7чтобы загрузить скрипт с вашего андроида", "&7в базу данных сервера");
        ItemMeta meta = it.getItemMeta();
        meta.setCustomModelData(1);
        it.setItemMeta(meta);
        menu.addItem(48, it);
        ...
        ItemStack it1 = new CustomItem(HeadTexture.SCRIPT_LEFT.getAsItemStack(), "&6> Назад", "", "&7Вернитесь к интерфейсу Андроида");
        meta = it1.getItemMeta();
        meta.setCustomModelData(1);
        it1.setItemMeta(meta);
        menu.addItem(53, it1);
    ...
    public void openScriptEditor(Player p, Block b) {
        ...
        menu.addItem(1, new CustomItem(HeadTexture.SCRIPT_FORWARD.getAsItemStack(), "&2> Изменить Скрипт", "", "&aОтредактируйте свой скрипт"));
        ...
        menu.addItem(3, new CustomItem(HeadTexture.SCRIPT_NEW.getAsItemStack(), "&4> Создать новый скрипт", "", "&cУдаляет ваш текущий скрипт", "&cи создает пустой"));
        menu.addMenuClickHandler(3, (pl, slot, item, action) -> {
            openScript(pl, b, DEFAULT_SCRIPT);
            return false;
        });

        menu.addItem(5, new CustomItem(HeadTexture.SCRIPT_DOWN.getAsItemStack(), "&6> Скачать скрипт", "", "&eЗагрузите скрипт с сервера", "&eВы можете редактировать или просто использовать его"));
        menu.addMenuClickHandler(5, (pl, slot, item, action) -> {
            openScriptDownloader(pl, b, 1);
            return false;
        });

        menu.addItem(8, new CustomItem(HeadTexture.SCRIPT_LEFT.getAsItemStack(), "&6> Назад", "", "&7Вернитесь к интерфейсу андроид"));
    ...
    protected void editInstruction(Player p, Block b, String[] script, int index) {
        ...
        menu.setEmptySlotsClickable(false);
        menu.addItem(9, new CustomItem(HeadTexture.SCRIPT_PAUSE.getAsItemStack(), "&fНичего не делать"), (pl, slot, item, action) -> {
            String code = deleteInstruction(script, index);
            setScript(b.getLocation(), code);
            openScript(p, b, code);
            return false;
    ...
    public void setScript(@Nonnull Location l, @Nonnull String script) {
        ...
        Validate.isTrue(script.startsWith(Instruction.START.name() + '-'), "Сценарий должен начинаться с маркера \"СТАРТ\".");
        Validate.isTrue(script.endsWith('-' + Instruction.REPEAT.name()), "Сценарий должен заканчиваться маркером \"ПОВТОРЯТЬ\".");
        Validate.isTrue(PatternUtils.DASH.split(script).length <= MAX_SCRIPT_LENGTH, "Сценарии могут содержать не более " + MAX_SCRIPT_LENGTH + " сегментов");
    ...
    private void registerDefaultFuelTypes() {
        switch (getFuelSource()) {
            ...
            case NUCLEAR:
                ...
                break;
            default:
                throw new IllegalStateException("Необработанный Источник Топлива: " + getFuelSource());
        }
    }
    ...
    public List<ItemStack> getDisplayRecipes() {
        ...
        for (MachineFuel fuel : fuelTypes) {
            ...
            lore.add(ChatColors.color("&8\u21E8 &7Осталось " + NumberUtils.getTimeLeft(fuel.getTicks() / 2)));//lasts
            im.setLore(lore);
            item.setItemMeta(im);
            list.add(item);
        }
    ...
    public void addItems(Block b, ItemStack... items) {
        Validate.notNull(b, "Блок не может быть пустым.");
    ...
    protected void attack(Block b, BlockFace face, Predicate<LivingEntity> predicate) {
        throw new UnsupportedOperationException("Этот тип андроида не умеет делать это!");
    }
