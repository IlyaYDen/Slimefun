...
public final class PostSetup {
    ...
    private static void addSmelteryRecipe(ItemStack[] input, ItemStack[] output, MakeshiftSmeltery makeshiftSmeltery) {
        ...
        if (ingredients.size() == 1 && isDust(ingredients.get(0))) {
            ...
            registerMachineRecipe("ELECTRIC_INGOT_FACTORY", 5, new ItemStack[] { ingredients.get(0) }, new ItemStack[] { output[0] });
            registerMachineRecipe("ELECTRIC_INGOT_PULVERIZER", 5, new ItemStack[] { output[0] }, new ItemStack[] { ingredients.get(0) });
        } else {
