...
public class GEOMiner extends SlimefunItem implements RecipeDisplayItem, EnergyNetComponent, InventoryBlock, HologramOwner, MachineProcessHolder<MiningOperation> {
    ...
    public final GEOMiner setCapacity(int capacity) {
        Validate.isTrue(capacity > 0, "Емкость должна быть больше нуля!");

        if (getState() == ItemState.UNREGISTERED) {
            this.energyCapacity = capacity;
            return this;
        } else {
            throw new IllegalStateException("Вы не можете изменить емкость после регистрации Предмета.");
        }
    }
    ...
    public final GEOMiner setProcessingSpeed(int speed) {
        Validate.isTrue(speed > 0, "Скорость должна быть больше нуля!");
    ...
    public final GEOMiner setEnergyConsumption(int energyConsumption) {
        Validate.isTrue(energyConsumption > 0, "Потребление энергии должно быть больше нуля!");
        Validate.isTrue(energyCapacity > 0, "Вы должны указать емкость, прежде чем сможете установить объем потребления.");
        Validate.isTrue(energyConsumption <= energyCapacity, "Потребление энергии не может быть выше, чем мощность (" + energyCapacity + ')');
    ...
    protected void tick(@Nonnull Block b) {
        ...
        if (operation != null) {
            ...
            updateHologram(b, "&4Требуется геосканирование!");
        } else {
            start(b, inv);
        }
    }
        ...
        for (GEOResource resource : SlimefunPlugin.getRegistry().getGEOResources().values()) {
            if (resource.isObtainableFromGEOMiner()) {
                ...
                if (supplies > 0) {
                    ...
                    updateHologram(b, "&7Добыча: &r" + resource.getName());
                    return;
                }
            }
        }

        updateHologram(b, "&7Закончено");
    }
