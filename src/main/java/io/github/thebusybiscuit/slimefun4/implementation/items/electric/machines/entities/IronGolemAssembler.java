...
public class IronGolemAssembler extends AbstractEntityAssembler<IronGolem> {
    ...
    protected void constructMenu(BlockMenuPreset preset) {
        preset.addItem(1, new CustomItem(getHead(), "&7Слот для Тыквы", "", "&fЭтот слот принимает Тыквы"), ChestMenuUtils.getEmptyClickHandler());
        preset.addItem(7, new CustomItem(getBody(), "&7Слот для Железных Блоков", "", "&fЭтот слот принимает Железные блоки"), ChestMenuUtils.getEmptyClickHandler());
        preset.addItem(13, new CustomItem(Material.CLOCK, "&7Охлаждение: &b30 Секунд", "", "&fРабота этой машины занимает до полуминуты."), ChestMenuUtils.getEmptyClickHandler());
    }
