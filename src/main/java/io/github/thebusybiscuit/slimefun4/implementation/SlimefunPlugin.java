...
public final class SlimefunPlugin extends JavaPlugin implements SlimefunAddon {
    ...
    public static boolean dedug = true;
    /**
    ...
    private void onPluginStart() {
        ...
        getLogger().log(Level.INFO, "Loading it ggg ems...");
        loadItems();
    ...
    //		{ "predicate": {"custom_model_data":1}, "model": "item/custom/items/sfitems/tree_tap/tree_tap2"},
    private void SetID(SlimefunPlugin slimefunPlugin) {
        Map<Material,String> st = new HashMap<>();
        int a = 0;
        String gf = "";
        ...
        for(SlimefunItem i : SlimefunPlugin.getRegistry().getAllSlimefunItems()){
            ...
            String c = i.getCategory().getUnlocalizedName().toLowerCase().replace(' ','_').replace('(','_').replace(')','_').replace('\'','_');

            String g =  "\n" + "\t\t{ \"predicate\": {\"custom_model_data\": " + a + "}, \"model\": \"item/custom/" +c + "/" + i.getId().toLowerCase() + "\"},";
            if(st.containsKey(i.getItem().getType()) && st.get(i.getItem().getType())!=null) {
                String s = st.get(i.getItem().getType());
                s = s + g;
                st.put(i.getItem().getType(),s);
            }
            else {
                st.put(i.getItem().getType(),"{\n" +
        ...
        for(Material m : Material.values()) {
            if(!st.containsKey(m)) continue;
            PrintWriter writer = null;
            try {
                writer = new PrintWriter("plugins/slimefun/rpa/" + m.toString().toLowerCase() + ".json", "UTF-8");
                writer.println(st.get(m) + "{}]}");
                writer.close();
        ...
        PrintWriter writer = null;
        try {
            writer = new PrintWriter("plugins/slimefun/rpa/models.yml", "UTF-8");
            writer.println(gf);
            writer.close();
        } catch (FileNotFoundException e) {
            e.printStackTrace();
