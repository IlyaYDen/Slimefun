...
public abstract class AbstractEnergyProvider extends SlimefunItem implements InventoryBlock, RecipeDisplayItem, EnergyNetProvider {
    ...
    @ParametersAreNonnullByDefault
    protected AbstractEnergyProvider(Category category, SlimefunItemStack item, RecipeType recipeType, ItemStack[] recipe, int t,String st) {
    ...
    public List<ItemStack> getDisplayRecipes() {
        ...
        for (MachineFuel fuel : fuelTypes) {
            ...
            lore.add(ChatColors.color("&8\u21E8 &7Продолжительность " + NumberUtils.getTimeLeft(fuel.getTicks() / 2)));//Lasts
            //lore.add(ChatColors.color("&8\u21E8 &7Осталось " + NumberUtils.getTimeLeft(fuel.getTicks() / 2)));//Lasts
            lore.add(ChatColors.color("&8\u21E8 &e\u26A1 &7" + getEnergyProduction() * 2) + " Дж/с");
            lore.add(ChatColors.color("&8\u21E8 &e\u26A1 &7" + NumberUtils.getCompactDouble((double) fuel.getTicks() * getEnergyProduction()) + " Дж в общей сложности"));
            im.setLore(lore);
            item.setItemMeta(im);
            list.add(item);
        }
