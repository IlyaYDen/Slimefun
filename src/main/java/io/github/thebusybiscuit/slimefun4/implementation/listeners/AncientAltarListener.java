...
public class AncientAltarListener implements Listener {
    ...
    public void onInteract(PlayerRightClickEvent e) {
        if(e.getInteractEvent().getHand() == EquipmentSlot.OFF_HAND) return;
        ...
        if (b.getType() != Material.ENCHANTING_TABLE && b.getType() != Material.GLASS) {
            return;
        }
    ...
    private void startRitual(Player p, Block b, List<Block> pedestals, ItemStack catalyst) {
        ...
        if (result.isPresent()) {
            if (SlimefunUtils.canPlayerUseItem(p, result.get(), true)) {
                ...
                SlimefunPlugin.runSync(task, 1L);
            } else {
                altars.remove(b);
