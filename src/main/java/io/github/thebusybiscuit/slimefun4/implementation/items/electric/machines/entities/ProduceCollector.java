...
public class ProduceCollector extends AContainer implements RecipeDisplayItem {
    ...
    public @Nonnull List<ItemStack> getDisplayRecipes() {
        ...
        displayRecipes.add(new CustomItem(Material.BUCKET, null, "&fТребует &bКорову &fпоблизости"));
        ...
        if (SlimefunPlugin.getMinecraftVersion().isAtLeast(MinecraftVersion.MINECRAFT_1_17)) {
            displayRecipes.add(new CustomItem(Material.BUCKET, null, "&fТребует &bКозла &fпоблизости"));
            displayRecipes.add(new ItemStack(Material.MILK_BUCKET));
        }

        displayRecipes.add(new CustomItem(Material.BOWL, null, "&fТребует &bГрибы &fпоблизости"));
