...
public interface EnergyNetComponent extends ItemAttribute {
    ...
    default void setCharge(@Nonnull Location l, int charge) {
        Validate.notNull(l, "Location was null!");
        Validate.isTrue(charge >= 0, "Вы можете установить только нулевой или более заряд!");
        //edited Validate.isTrue(charge >= 0, "You can only set a charge of zero or more!");
    ...
    default void addCharge(@Nonnull Location l, int charge) {
        Validate.notNull(l, "Location was null!");
        Validate.isTrue(charge > 0, "Вы можете добавить только положительный заряд!");
