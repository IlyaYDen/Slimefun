...
public class MachineProcessor<T extends MachineOperation> {
    ...
    public void setProgressBar(@Nullable ItemStack progressBar) {
        ...
        ItemStack item = new CustomItem(Material.DIAMOND_PICKAXE," ");
    ...
    //edited
    public void updateProgressBarCMD(@Nonnull BlockMenu inv, int slot,int cmd,@Nonnull T operation) {
        Validate.notNull(inv, "The inventory must not be null.");
        Validate.notNull(operation, "The MachineOperation must not be null.");
        ...
        int remainingTicks = operation.getRemainingTicks();
        int totalTicks = operation.getTotalTicks();
        ChestMenuUtils.updateProgressbarCMD(inv, slot, remainingTicks, totalTicks, getProgressBar(),cmd);
    }


    public void updateProgressBarCMDI(@Nonnull BlockMenu inv, int slot,int cmd1,int cmd2,@Nonnull T operation) {
        Validate.notNull(inv, "The inventory must not be null.");
        Validate.notNull(operation, "The MachineOperation must not be null.");
        ...
        int remainingTicks = operation.getRemainingTicks();
        int totalTicks = operation.getTotalTicks();
        ChestMenuUtils.updateProgressbarCMDI(inv, slot, remainingTicks, totalTicks, getProgressBar(), cmd1, cmd2);
    }
    //edited end
}
