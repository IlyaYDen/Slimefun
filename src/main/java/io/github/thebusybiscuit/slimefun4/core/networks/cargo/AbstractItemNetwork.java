...
abstract class AbstractItemNetwork extends Network {
    ...
    private final ItemStack terminalPlaceholderItem = new CustomItem(Material.BARRIER, "&4Нет кэшированного элемента");
    ...
    private void updateTerminal(Location l, BlockMenu terminal, int slot, int index, List<ItemStackAndInteger> items) {
        if (items.size() > index) {
            ...
            if (stack.getMaxStackSize() > 1) {
                ...
                lore.add(ChatColors.color("&7Левый Клик: Запросить 1 | Правый Клик: Запросить " + amount + ">"));
            } else {
                lore.add(ChatColors.color("&7<7Левый Клик: Запросить 1>"));
            }
