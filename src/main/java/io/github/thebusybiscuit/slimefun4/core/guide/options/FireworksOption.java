...
class FireworksOption implements SlimefunGuideOption<Boolean> {
    ...
    public Optional<ItemStack> getDisplayItem(Player p, ItemStack guide) {
        if (SlimefunPlugin.getRegistry().isResearchFireworkEnabled()) {
            ...
            ItemStack item = new CustomItem(Material.FIREWORK_ROCKET, "&bФейерверки: &" + (enabled ? "aДа" : "4Нет"), "", "&7Вы можете переключить,", "&7будет ли феерверк появляться", "&7при исследовании предмета.", "", "&7\u21E8 &eНажмите, чтобы " + (enabled ? "отключить" : "включить") + " феерверки");
