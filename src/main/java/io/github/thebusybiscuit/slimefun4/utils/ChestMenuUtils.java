...
public final class ChestMenuUtils {
    ...
    private static final ItemStack UI_NULL = new SlimefunItemStack("_UI_NULL", Material.DIAMOND_PICKAXE, " ");

    private static final ItemStack UI_BACKGROUND = new SlimefunItemStack("_UI_BACKGROUND", Material.GRAY_STAINED_GLASS_PANE, " ");
    private static final ItemStack INPUT_SLOT = new SlimefunItemStack("_UI_INPUT_SLOT", Material.GRAY_STAINED_GLASS_PANE/*CYAN_STAINED_GLASS_PANE*/, " ");
    private static final ItemStack OUTPUT_SLOT = new SlimefunItemStack("_UI_OUTPUT_SLOT", Material.GRAY_STAINED_GLASS_PANE/*ORANGE_STAINED_GLASS_PANE*/, " ");
    private static final ItemStack PROSES_TEXTURE = new SlimefunItemStack("_UI_PROSES_SLOT", Material.GRAY_STAINED_GLASS_PANE/*ORANGE_STAINED_GLASS_PANE*/, " ");
    ...
    private static final ItemStack BACK_BUTTON = new SlimefunItemStack("_UI_BACK", Material.BLACK_STAINED_GLASS_PANE, "&7\u21E6 Back", meta -> meta.addItemFlags(ItemFlag.HIDE_ENCHANTS));
    ...
    public static @Nonnull ItemStack getNull() {
        return UI_NULL;
    }
    ...
    public static @Nonnull ItemStack getProsesTexture() {
        return PROSES_TEXTURE;
    }
    ...
    public static void updateProgressbar(@Nonnull ChestMenu menu, int slot, int timeLeft, int time, @Nonnull ItemStack indicator) {
        ...
        im.setDisplayName(" ");
        im.setLore(Arrays.asList(getProgressBar(timeLeft, time), "", ChatColor.GRAY + NumberUtils.getTimeLeft(timeLeft / 2) + " осталось"));
        item.setItemMeta(im);

        menu.replaceExistingItem(slot, item);
    }
    public static void updateProgressbarCMD(@Nonnull ChestMenu menu, int slot, int timeLeft, int time, @Nonnull ItemStack indicator, int cmd) {
        Inventory inv = menu.toInventory();
        ...
        im.setDisplayName(" ");
        if(SlimefunPlugin.dedug)
            im.setLore(Arrays.asList(getProgressBar(timeLeft, time), "", ChatColor.GRAY + NumberUtils.getTimeLeft(timeLeft / 2) + " осталось","","CMD (нач " + cmd + "): " + (cmd - (1+ time-timeLeft))));

        im.setCustomModelData(cmd - (1+ time-timeLeft));
        item.setItemMeta(im);

        menu.replaceExistingItem(slot, item);
    }
    public static void updateProgressbarCMDI(@Nonnull ChestMenu menu, int slot, int timeLeft, int time, @Nonnull ItemStack indicator, int cmd1, int cmd2) {
        Inventory inv = menu.toInventory();
        ...
        if(SlimefunPlugin.dedug)
            im.setLore(Arrays.asList(getProgressBar(timeLeft, time), "", ChatColor.GRAY + NumberUtils.getTimeLeft(timeLeft / 2) + " осталось",""
                    ,"CMD (нач " + cmd1 + "): " + (1000+(int)((double)(t/time) * (time - timeLeft))),
                    "t = " + t,
                    "time= "+ time,
                    "timeLeft = " + timeLeft
            ));
        ...
        im.setCustomModelData(1000+(int)((double)(t/time) * (timeLeft)));//cmd1+(int)((double)(t/time) * (time - timeLeft))
        //(short) ((item.getType().getMaxDurability() / max) * timeLeft);
        if(SlimefunPlugin.dedug)
            im.setLore(Arrays.asList(getProgressBar(timeLeft, time), "", ChatColor.GRAY + NumberUtils.getTimeLeft(timeLeft / 2) + " осталось",""
                    ,"CMD (нач " + cmd1 + "): " + (1000+(int)((double)(t/time) * (timeLeft))),
                    "t = " + t,
                    "time= "+ time,
                    "timeLeft = " + timeLeft,
