...
public abstract class AGenerator extends AbstractEnergyProvider implements MachineProcessHolder<FuelOperation> {
    ...
    @ParametersAreNonnullByDefault
    protected AGenerator(Category category, SlimefunItemStack item, RecipeType recipeType, ItemStack[] recipe, int slot, int cmd, int[] in, int[] out,int t, String st) {
    ...
    public int getGeneratedOutput(Location l, Config data) {
        ...
        if (operation != null) {
            if (!operation.isFinished()) {
                if(cmd==0)
                processor.updateProgressBar(inv, slot, operation);else {

                    ItemStack item = new CustomItem(getProgressBar(), " ");
                    ItemMeta meta = item.getItemMeta();
                    meta.setCustomModelData(cmd-1);
                    item.setItemMeta(meta);
