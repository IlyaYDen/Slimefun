...
//added close
    ...
    // edited
    @ParametersAreNonnullByDefault
    public SlimefunItem(Category category, SlimefunItemStack item, RecipeType recipeType, ItemStack[] recipe,int t, String st) {
        this(category, item, recipeType, recipe, null,t,st);
        Bukkit.broadcastMessage(item.getItemId() + " fff ");
    }
    ...
    public SlimefunItem(Category category, SlimefunItemStack item, RecipeType recipeType, ItemStack[] recipe, @Nullable ItemStack recipeOutput) {
        ...
        if(SlimefunPlugin.dedug) {
            ...
            if(meta.hasCustomModelData()){
                Lore.add(ChatColor.WHITE + "CATEGORY: " + category.getUnlocalizedName());
                Lore.add(ChatColor.WHITE + "SFID: " + item.getItemId());
                Lore.add(ChatColor.WHITE + "MCID: " + item.getType());
                Lore.add(ChatColor.WHITE + "CMD: " + meta.getCustomModelData());
            }
            else{
                Lore.add(ChatColor.WHITE + "CATEGORY: " + category.getUnlocalizedName());
                Lore.add(ChatColor.WHITE + "SFID: " + item.getItemId());
                Lore.add(ChatColor.WHITE + "MCID: " + item.getType());
                Lore.add(ChatColor.WHITE + "CMD: - " );
            }
            meta.setLore(Lore);
        ...
        //"plugins/slimefun/rp/resources/" + category +  "/" + item.getItemId().toLowerCase() + ".yml"
        if(SlimefunPlugin.dedug) {
            System.out.print("plugins//slimefun//rp//resources/" + category +  "/" + item.getItemId().toLowerCase() + ".yml");
        }
        String a = category.getUnlocalizedName().toLowerCase().replace(' ','_').replace('(','_').replace(')','_').replace('\'','_');

        //File f = new File("plugins\\slimefun\\rp\\resources\\" + category +  "\\" +  item.getItemId().toLowerCase() + ".yml");
        //f.mkdir();
        String gen = "item/generated";
        ...
        String json = "\n" +
                ...
                "    \"parent\": \""+ gen + "\",\n" +
                "    \"textures\": {\n" +
                "    \"layer0\": \"slimefun/" + a + "/" + id.toLowerCase() + "\"\n" +
                "    }\n" +
                "}\n";
        ...
        File fd = new File("plugins/slimefun/rp");
        fd.mkdir();
        fd = new File("plugins/slimefun/rp/" + a + "/" + id.toLowerCase());
        fd.mkdir();
        File fdd = new File(fd,"test.t");
        PrintWriter writer = null;
        try {
            writer = new PrintWriter("plugins/slimefun/rp/" + a + "/" + id.toLowerCase()+".json", "UTF-8");
            writer.println(json);
            writer.close();
    ...
    @ParametersAreNonnullByDefault
    public SlimefunItem(Category category, SlimefunItemStack item, RecipeType recipeType, ItemStack[] recipe, @Nullable ItemStack recipeOutput,int t,String st) {
        Validate.notNull(category, "'category' is not allowed to be null!");
        Validate.notNull(item, "'item' is not allowed to be null!");
        Validate.notNull(recipeType, "'recipeType' is not allowed to be null!");
    ...
    protected SlimefunItem(Category category, ItemStack item, String id, RecipeType recipeType, ItemStack[] recipe) {
        ...
        this.recipeType = recipeType;
        this.recipe = recipe;
        Bukkit.broadcastMessage(id + " fff ");
    }
    ...
    //armor
    @Nonnull
    public final short getArmor() {
        return armor;
    }
    @Nonnull
    public final String getBreakItem() {
        return BreakItem;
    }
    @Nonnull
